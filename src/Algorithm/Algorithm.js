...
function Algorithm() {
  ...
  return (
    <>
      ...
      <p>Best: {currentAlgorithm.best_plus_penalty < 100000 && helpers.fmtMSS(currentAlgorithm.solve_time_best)}</p>
      {currentAlgorithm.solve_time_penalty > 0 ?
      <span className="penalty-time">{`+${currentAlgorithm.solve_time_penalty}m penalty`}<img className="red-bolt" src={require('../Img/redboltarrow.png')} alt=""/></span>
      : <br/> }
      <p>Current: <span> {context.currentSolveTime}</span></p>
      {context.currentPenalty && <>
      <p className="penalty-time">{context.currentPenalty}<span><img className="red-bolt" src={require('../Img/redboltarrow.png')} alt=""/></span></p>
      </>
}
    </>
  );
